...
app.post('/api/customers', async (req, res) => {
    try {
        ...
        const mappedPolicies = policies ? policies.map((p: any) => {
            const { customerId, ...rest } = p;
            return rest;
        }) : [];
        const customer = await prisma.customer.create({
            data: { ...customerData, policies: { create: mappedPolicies } },
            ...
            if (owners.length > 0) {
                await prisma.notification.createMany({
                    data: owners.map(owner => ({
                        userId: owner.id,
                        title: 'New Customer Pending',
                        message: `A new customer ${customer.name} requires your approval.`,
                        type: 'approval',
                        link: `/owner/customers/${customer.id}`
                    }))
                });
            }
        }
        res.json(customer);
    } catch (error) { console.error(error); res.status(500).json({ error: 'Failed to create customer' }); }
});
